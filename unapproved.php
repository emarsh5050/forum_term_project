...
<?php
	...
	$sql = "SELECT      Thread.TID, Thread.Title, Thread.PostDate, Thread.FID, Thread.FPID, Post.PID, Post.content
            FROM       Thread, Post
            WHERE      Thread.FID = 1 and Post.ApprovalStatus = 0 and Post.Rejected =0 and Post.TID = Thread.TID;";
	...
	if ($stmt->prepare($sql)){
		...
		$stmt->execute();
		$stmt->bind_result($TID, $Title, $PostDate, $FID, $FPID, $PID, $PContent);
		...
		while($stmt->fetch()){
            ...
            if($lastTID != $TID){$tblRows = $tblRows."<tr><td></td>";
            
            $tblRows = $tblRows."<td>Thread:</td><td></td>";
			$tblRows = $tblRows."<td><a href='ViewThread.php?TID=$TID'>$Title</a></td>";
				/*<td><a href='javascript:confirmDel(\"$Title_js\",$TID)'>Delete</a> </td>*/
			$tblRows = $tblRows."<td>$PostDate</td></tr> <tr><td></td><td>Posts:</td><td></td><td></td><td></td></tr>"; 
            ...
            $tblRows = $tblRows."<td><a href='unapprovePost.php?PID=$PID'>Reject</a></td><td><a href='javascript:confirmDel(\"$Title\",$TID)'>Delete</a></td>";
			$tblRows = $tblRows."<td><a href='ViewThread.php?TID=$TID'>$PContent</a></td>";
				/*<td><a href='javascript:confirmDel(\"$Title_js\",$TID)'>Delete</a> </td>*/
		...
		$output = "
				<div class=\"title\">
		<h2  class=\"title\">Unapproved Thread and Post List</h2>
		</div>
