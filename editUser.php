...
if (isset($_POST['Submit'])) {
	...
	$required = array("UserName", "Password","EmailAdress"); // note that, in this array, the spelling of each item should match the form field names

	// set up the expected array
	$expected = array("UID", "UserName", "Password","EmailAdress", "StreetAdress", "City","State", "Zip", "Admin","ParentID", "ForumAcess"); // again, the spelling of each item should match the form field names
    
    // set up a label array, use the field name as the key and label as the value
    $label = array ('UID'=>'UID', 'UserName'=>'UserName', "Password"=>'Password', 'EmailAdress'=>'EmailAdress', 'ForumAcess'=>'ForumAcess');
	...
	if (empty($missing)){
		...
		if ($UID != "") {
			...
			$sql = "UPDATE `Users` SET `UserName`=?,`Password`=?,`EmailAdress`=?,`StreetAdress`=?,`City`=?,`State`=?,`Zip`=?,`Admin`=?,`ParentID`=?,`ForumAcess`=? WHERE UID=?";
			...
			if($stmt->prepare($sql)){
				...
				$stmt->bind_param('ssssssiiiii', $UserName, $Password, $EmailAdress, $StreetAdress, $City, $State, $Zip, $Admin, $ParentID, $ForumAcess, $UID);
		    ...
		    if(!isset($_POST[$ParentID])){ $ParentID = 0;}
			// no existing pid ==> this means no existing record to deal with, then it must be a new record ==> use an insert query
			$sql = "INSERT INTO `Users`(`UserName`, `Password`, `EmailAdress`,`UserImageURL`, `StreetAdress`, `City`, `State`, `Zip`, `Admin`, `ParentID`, `ForumAcess`) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";

			if($stmt->prepare($sql)){

				// Note: user input could be an array, the code to deal with array values should be added before the bind_param statment.

				$stmt->bind_param('sssssssiiii', $UserName, $Password, $EmailAdress, $UserImageURL, $StreetAdress, $City, $State, $Zip, $Admin, $ParentID, $ForumAcess);
				$stmt_prepared = 1; // set up a variable to signal that the query statement is successfully prepared.
			}
		}
		...
		$output = "<div class='center'><p>The following required fields are missing in your form submission.  Please check your form again and fill them out.  <br>Thank you.<br>\n<ul>\n";
		foreach($missing as $m){
			$output .= "<li>{$label[$m]}</li>\n";
		}
		
		if($UID){
		$output .= "</ul><br><a class=\"btn btn-secondary\" href=\"newUserForm.php\">Go back</a></div>\n";}
		else{
	...
	$output = "<p>User sucessfully edited</p>";
}
