...
if (isset($_POST['Submit'])) {
	...
	$required = array("anchorText", "URL", "categoryID"); // note that, in this array, the spelling of each item should match the form field names

	// set up the expected array
	$expected = array("anchorText", "URL", "categoryID", "lid"); // again, the spelling of each item should match the form field names
    
    // set up a label array, use the field name as the key and label as the value
    $label = array ('anchorText'=>'Anchor Text', "URL"=>'URL', "categoryID"=>'Category ID', 'lid'=>'Link ID',);
	...
	//echo("Anchor text: $anchorText, URL: $URL, CID: $categoryID");
	...
	if (empty($missing)){
		...
		if ($lid != "") {
			...
			$sql = "UPDATE `Links` SET `URL`=?,`AnchorText`=?,`CID`=? WHERE LinkID = ?";
				
			//echo("URL: $URL, AT: $anchorText, CID: $categoryID, LID: $lid");
			
			if($stmt->prepare($sql)){

				// Note: user input could be an array, the code to deal with array values should be added before the bind_param statment.
				$stmt->bind_param('ssii',$URL, $anchorText, $categoryID, $lid);
			...
			$sql = "Insert Into Links (URL, AnchorText, CID) values (?, ?, ?)";

			if($stmt->prepare($sql)){

				// Note: user input could be an array, the code to deal with array values should be added before the bind_param statment.

				$stmt->bind_param('ssi',$URL, $anchorText, $categoryID);
				$stmt_prepared = 1; // set up a variable to signal that the query statement is successfully prepared.
			}
		}
